...
#' @export
tsps <- function(formula, instruments, data, subset, na.action, contrasts = NULL, t0 = NULL, link = "identity", ...) {
  ...
  if (has_dot(formula)) {
    ...
    if (!has_dot(f1) && has_dot(f2)) formula <- Formula::as.Formula(
      f1,
  ...
  # check y binary
  if (link == "logit" && !all(Y %in% 0:1)) stop(
  ...
  if (is.null(t0)) {
    ...
    if (link == "identity") {
      ...
      Ystar <- Y
      Ystar[Y == 0] <- 0.001
      stage2 <- stats::glm(Ystar ~ xhat, family = stats::Gamma(link = "log"), control = list(maxit = 1E5))
    } else if (link == "logit") {
  ...
  tsps_gmm <- function(x, y, z, xnames, t0, link) {
    ...
    if (fit$algoInfo$convergence != 0) warning(
  ...
  tspsIdentityMoments <- function(theta, x) {
    ...
    moments[, stage2start] <- (
      Y - (theta[stage2start] + as.matrix(stage2linpred) %*% as.matrix(theta[thetastart:thetaend]))
    )

    start3 <- stage2start + 1
    j <- 1
    for (i in start3:thetaend) {
      moments[, i] <- (Y - (theta[stage2start] + as.matrix(stage2linpred) %*% as.matrix(theta[thetastart:thetaend]))) *
        xhat[, j]
      j <- j + 1
    }
  ...
  tspsLogaddMoments <- function(theta, x) {
    ...
    thetastart <- stage2start + 1
    moments[, stage2start] <- (
      Y - exp(theta[stage2start] + as.matrix(stage2linpred) %*% as.matrix(theta[thetastart:thetaend]))
    )

    start3 <- stage2start + 1
    j <- 1
    for (i in start3:thetaend) {
      moments[, i] <- (Y - exp(theta[stage2start] + as.matrix(stage2linpred) %*% as.matrix(theta[thetastart:thetaend]))) *
        xhat[, j]
      j <- j + 1
    }
  ...
  tspsLogmultMoments <- function(theta, x) {
    ...
    thetastart <- stage2start + 1
    moments[, stage2start] <- (
      (Y * exp(-1 * (theta[stage2start] + as.matrix(stage2linpred) %*% as.matrix(theta[thetastart:thetaend])))) - 1
    )
  ...
  tspsLogitMoments <- function(theta, x) {
    ...
    thetastart <- stage2start + 1
    moments[, stage2start] <- (
      Y - stats::plogis(theta[stage2start] + as.matrix(stage2linpred) %*% as.matrix(theta[thetastart:thetaend]))
    )
  ...
  # gmm fit
  output <- tsps_gmm(x = Xtopass, y = Y, z = Ztopass, xnames = xnames, t0 = t0, link = link)
...
print.summary.tsps <- function(x, digits = max(3, getOption("digits") - 3), ...) {
  ...
  if (x$object$link %in% c("logadd", "logmult", "logit")) {
    parname <- "Causal odds ratio"
    if (x$object$link %in% c("logadd", "logmult")) parname <- "Causal risk ratio"
    cat("\n", parname, " with 95% CI limits:\n", sep = "")
