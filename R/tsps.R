...
tsps <- function(formula, instruments, data, subset, na.action,
  ...
  if (is.null(t0)) {
    ...
    if (link == "identity") {
      ...
      stage2 <- fastglm::fastglm(Y ~ xhat, family = stats::poisson(link = "log"))
    } else if (link == "logmult") {
      Ystar <- Y
      Ystar[Y == 0] <- 0.001
      stage2 <- fastglm::fastglm(Ystar ~ xhat, family = stats::Gamma(link = "log"),
                    control = list(maxit = 1E5))
    } else if (link == "logit") {
      stage2 <- fastglm::fastglm(Y ~ xhat, family = stats::binomial(link = "logit"))
    }
