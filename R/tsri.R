...
tsri <- function(formula, instruments, data, subset, na.action,
  ...
  if (is.null(t0)) {
    ...
    if (link == "identity") {
      ...
      stage2 <- parglm::parglm(Y ~ X[, 2] + res, family = stats::poisson(link = "log"))
    } else if (link == "logmult") {
      Ystar <- Y
      Ystar[Y == 0] <- 0.001
      stage2 <- parglm::parglm(Ystar ~ X[, 2] + res, family = stats::Gamma(link = "log"),
                    control = list(maxit = 1E5))
    } else if (link == "logit") {
      stage2 <- parglm::parglm(Y ~ X[, 2] + res, family = stats::binomial(link = "logit"))
    }
