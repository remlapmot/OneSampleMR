...
test_that("Results are invariant to the order of terms in the formula", {
  ...
  fit40 <- tsri(Y ~ X + C1 + C2 | G1 + G2 + G3 + C1 + C2, data = dat)
  fit41 <- tsri(Y ~ C1 + C2 + X | C1 + C2 + G1 + G2 + G3, data = dat)
  fit42 <- tsri(Y ~ C1 + X + C2 | G1 + C1 + G2 + C2 + G3, data = dat)
