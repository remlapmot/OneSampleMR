...
```{r}
fit02 <- msmm(Y ~ X | Z, data = dat, t0 = rep(.1, 2))
summary(fit02)
```
...
```{r}
fit03 <- msmm(Y ~ X | Z, data = dat, t0 = rep(.1, 2), estmethod = "gmmalt")
summary(fit03)
```
...
```{r}
fit04 <- msmm(Y ~ X | Z, data = dat, t0 = rep(.1, 2), estmethod = "tsls")
summary(fit04)
```
...
```{r}
fit05 <- msmm(Y ~ X | Z, data = dat, t0 = rep(.1, 2), estmethod = "tslsalt")
summary(fit05)
```
